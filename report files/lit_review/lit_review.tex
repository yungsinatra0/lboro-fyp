...
Multiple research papers recognise interviews as the most commonly used technique for requirement gathering \parencite{interviews5,interviews1,interviews2}. Consequently, some studies have looked at best practices and common mistakes when conducting requirement gathering interviews - the results are summarized in the tables below.
%TODO: Compare these 2 studies 
\begin{table}[h!]
    \centering
    \small
    \renewcommand{\arraystretch}{1.2}
    \begin{tabular}{|>{\arraybackslash}m{0.2\textwidth}|>{\arraybackslash}m{0.75\textwidth}|}
    \hline
...
The success of LLMs depends not only on the model itself - but also on how it's effectively used by the users, using techniques like prompt engineering, which involves the constant designing and refining of prompts to guide the output of LLMs \parencite{promptmed,prompt2}. Prompts represent instructions given to the model to guide its output, such as providing context, examples, or constraints to the model \parencite{prompt,prompt1}. 

There are multiple techniques for prompt engineering, ranging from simple to more advanced. The table below outlines some of the most common techniques, as outlined by \textcite{prompt1, prompt2}.

\begin{table}[h!]
    \centering
    \begin{tabular}{p{3cm} p{7cm} p{3cm}}
        \toprule
        \textbf{Name of the Technique} & \textbf{Short Description} & \textbf{Source} \\
        \midrule
        Chain-of-Thought Prompting     & Encourages the model to break down complex problems into intermediate reasoning steps before producing an answer. & test \\
        Few-Shot Prompting             & Provides the model with a few input-output examples in the prompt to guide its predictions for new queries. & test \\
        Self-Consistency               & Uses multiple sampled outputs to determine the most consistent and accurate final prediction. & test \\
        Zero-Shot Prompting            & Directly prompts the model to perform a task without providing examples, relying solely on task instructions. & test \\
        Instruction Tuning             & Fine-tunes models on large datasets of instruction-response pairs to make them follow instructions better. & test \\
        \bottomrule
    \end{tabular}
    \caption{Prompt Engineering Techniques from Academic Sources}
